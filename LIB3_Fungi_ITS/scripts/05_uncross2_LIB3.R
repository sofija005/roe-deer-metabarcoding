...
# Load non-chimeric table
seqtab.nochim <- readRDS(
  file.path(nochim_path, "LIB3_seqtab_nochim.rds")
)

# Load original ASV IDs
raw_map <- fread(
...
fwrite(
  ASVTAB,
  file.path(out_path, "LIB3_UNCROSS2_scores.tsv"),
  sep = "\t"
)
...
fwrite(
  filtered_long,
  file.path(out_path, "LIB3_UNCROSS2_filtered_long.tsv"),
  sep = "\t"
)
...
fwrite(
  filtered_wide,
  file.path(out_path, "LIB3_ASV_table_uncross2.csv"),
  sep = ","
)

# Keep matching sequences
...
writeLines(
  fasta_lines,
  file.path(out_path, "LIB3_ASVs_uncross2.fasta")
)

# Save DADA2-style matrix
...
colnames(seqtab.uncross2) <- kept_map$Sequence

saveRDS(
  seqtab.uncross2,
  file.path(out_path, "LIB3_seqtab_uncross2.rds")
)
...
fwrite(
  summary,
  file.path(out_path, "LIB3_UNCROSS2_summary.tsv"),
  sep = "\t"
)

cat("\n============================================\n")
cat("LIB3 UNCROSS2 COMPLETED\n")
cat("============================================\n")
