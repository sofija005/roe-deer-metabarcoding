...
# ------------------------------------------------------------
# Find filtered paired-end reads
# ------------------------------------------------------------
...
saveRDS(
  errF,
  file.path(error_path, "LIB1_errF.rds")
)

saveRDS(
  errR,
  file.path(error_path, "LIB1_errR.rds")
)
...
pdf(file.path(error_path, "LIB1_error_rates_R1.pdf"))
plotErrors(errF, nominalQ = TRUE)
dev.off()

pdf(file.path(error_path, "LIB1_error_rates_R2.pdf"))
...
saveRDS(
  dadaFs,
  file.path(intermediate_path, "LIB1_dadaFs.rds")
)

saveRDS(
  dadaRs,
  file.path(intermediate_path, "LIB1_dadaRs.rds")
)
...
saveRDS(
  mergers,
  file.path(intermediate_path, "LIB1_mergers.rds")
)
...
# ASV abundance table: ASVs as rows, samples as columns
...
write.csv(
  asv_table,
  file.path(raw_path, "LIB1_ASV_table_raw.csv"),
  row.names = TRUE
)
...
writeLines(
  fasta_lines,
  file.path(raw_path, "LIB1_ASVs_raw.fasta")
)
...
rownames(filter_summary) <- filter_summary$sample

tracking <- data.frame(
  Sample = samples,
  Input = filter_summary[samples, "reads.in"],
  Quality_filtered = filter_summary[samples, "reads.out"],
  Denoised_R1 = denoisedF[samples],
  Denoised_R2 = denoisedR[samples],
...
write.csv(
  tracking,
  file.path(tracking_path, "LIB1_tracking_part1.csv"),
  row.names = FALSE
)
...
cat("\n============================================\n")
cat("LIB1 DADA2 PART 1 COMPLETED\n")
cat("============================================\n")
...
cat(
  ...
  round(
    100 * sum(tracking$Merged) /
      min(sum(tracking$Denoised_R1), sum(tracking$Denoised_R2)),
    2
  ),
  "%\n"
)
