...
# Match sequences to the original stable ASV IDs
...
# ------------------------------------------------------------
# Convert to ASV x sample abundance table
# ------------------------------------------------------------
...
setcolorder(wide, c("ASV", setdiff(names(wide), "ASV")))
...
# ------------------------------------------------------------
# UNCROSS2 score function from Bioscanflow
# ------------------------------------------------------------
...
fwrite(
  ASVTAB,
  file.path(out_path, "LIB1_UNCROSS2_scores.tsv"),
  sep = "\t"
)
...
fwrite(
  filtered_long,
  file.path(out_path, "LIB1_UNCROSS2_filtered_long.tsv"),
  sep = "\t"
)
...
fwrite(
  filtered_wide,
  file.path(out_path, "LIB1_ASV_table_uncross2.csv")
)
...
# ------------------------------------------------------------
# FASTA with IDs matching abundance table
# ------------------------------------------------------------
...
writeLines(
  fasta_lines,
  file.path(out_path, "LIB1_ASVs_uncross2.fasta")
)


# ------------------------------------------------------------
# Save DADA2-style RDS table
# samples x DNA sequences
# ------------------------------------------------------------
...
saveRDS(
  seqtab.uncross2,
  file.path(out_path, "LIB1_seqtab_uncross2.rds")
)
...
fwrite(
  summary,
  file.path(out_path, "LIB1_UNCROSS2_summary.tsv"),
  sep = "\t"
)
