...
write.csv(
  nochim_map,
  file.path(nochim_path, "LIB2_ASV_sequences_nochim.csv"),
  row.names = FALSE
)
...
write.csv(
  nochim_table,
  file.path(nochim_path, "LIB2_ASV_table_nochim.csv"),
  row.names = TRUE
)
...
writeLines(
  fasta_lines,
  file.path(nochim_path, "LIB2_ASVs_nochim.fasta")
)
...
write.csv(
  tracking,
  file.path(nochim_path, "LIB2_chimera_tracking.csv"),
  row.names = FALSE
)
...
cat("\n============================================\n")
cat("LIB2 CHIMERA REMOVAL COMPLETED\n")
cat("============================================\n")
